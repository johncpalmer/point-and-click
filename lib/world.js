...
// Keyed by node id. Each node with an `interactable` has a locked and unlocked
// outcome, played by POST /api/interact depending on whether the player carries
// the required object.
...
export const INTERACTION_RESULTS = {
  tuningroom: {
    locked: {
      ...
      text: "The socket in the pin's head is square, cold, and empty. Somewhere on Cadence there is a key that was made to fill it — heavy, brass, humming. The room has waited thirty-nine years; it can wait for you.",
    },
    unlocked: {
      title: 'THE ANSWERING',
      text: "You seat the brass key in the socket. It stops humming — and for the first time in thirty-nine years, so does the island. Then, in the silence you have made, you feel it through the soles of your feet: the sea, asking. One note, older than memory, patient as tide. The key turns almost by itself. Every string on Cadence tightens by one held breath. Every pipe opens. The cut measure — hidden all these years in cords and clappers and careful silence — plays itself, and the island answers the sea at last. Far past the horizon, something that has been listening longer than the Tidewrights were alive rises very slightly, like a listener leaning forward. And in the hum that follows — fuller now, and no longer fading — you can hear them plainly: the Tidewrights, still holding their note, no longer alone.",
    },
  },
};

// --- Authored item uses --------------------------------------------------------
// Keyed `${objectId}@${nodeId}`. Applying a use may consume the object, change
// the node's world state (repainting the scene), grant a caretaker's clue, and
// carry a journalNote returned as a one-off toast (never added to the fragments
// journal). Uses resolve identically in mock and live — no LLM needed.
// `event` (optional) records a non-repainting world-state marker so caretakers
// can acknowledge the deed in chat (see EVENT_MAP in server.js).

export const USES = {
  'clapper@bellchamber': {
    title: 'THE BELL REMEMBERS',
    text: "You lift the clapper into the empty mouth and seat its hook. For a moment, nothing. Then the bell leans — by itself, the width of a breath — and rings once, clean and enormous, out over the whole of Cadence. Thirty-nine years of silence, spent in a single note.",
    consumes: true,
    stateChange: 'rung',
    journalNote: 'The bell of the Ringwood rang. Everyone on the island heard it.',
  },
  'mirror@hornmouth': {
    title: 'YOU FLASH THE HORIZON',
    text: "You catch the sun and throw it, three long, two short — Serak's own signature, stolen from his tally wall. The light goes out across the water. You count four heartbeats. Then, at the exact line where sea meets sky: an answer. Three long. Two short. Reversed.",
    consumes: false,
    stateChange: 'signaled',
    journalNote: 'Something across the sea answered the mirror. Immediately.',
  },
  'lens@pool': {
    title: 'FACES IN THE WATER',
    text: 'Through the green glass the black water is not black. It is full. Rows of faces hang in it like lanterns sunk in a line — eyes closed, mouths open, holding one note you feel in your teeth. One of them, near the surface, has Maren\'s jaw and Maren\'s brow, thirty-nine years younger.',
    consumes: false,
    event: 'seen',
    journalNote: 'Through the sea-glass lens, the listening pool is full of faces. The Tidewrights, mid-note.',
  },
  'smooth_stone@hollow': {
    title: "ODO'S TRADE",
    text: 'Odo takes the stone with both careful hands and holds it to the light like an egg. "Oh," he says. "Oh, you found the quiet one." He tucks it away and, for once, does not hum — he speaks the truth straight through, as payment.',
    consumes: true,
    grantsClue: 'odo',
    journalNote: null,
  },
  'cord@nave': {
    title: 'THE LAST KNOT, READ ALOUD',
    text: 'Lys takes the severed cord as if it might still bleed. Her fingers walk it twice. "Measure forty," she says finally. "The one we never played." She does not ask where you found it. She asks what you intend to do with the room it belongs to.',
    consumes: false,
    event: 'read',
    journalNote: 'Sister Lys read the severed measure aloud. She knows what you are carrying toward.',
  },
};
...
export const NODES = {
  ...
  bellchamber: {
    ...
    states: {
      rung: {
        addendum:
          'The bronze clapper now hangs inside the great bell, which is caught mid-swing; leaves shiver loose from the canopy around the belfry.',
      },
    },
  },
  hollow: {
  ...
  tuningroom: {
    ...
    interactable: {
      id: 'pin',
      name: 'the brass tuning pin',
      description:
        'A single pin at the heart of the instrument, its square socket empty, every string and pipe on Cadence leaning toward it.',
      position: 'rising waist-high at the exact center of the round chamber',
      requires: 'tuning_key',
    },
  },
  ...
  hornmouth: {
    ...
    states: {
      signaled: {
        addendum:
          'Far out on the horizon line, a pinpoint of answering light flashes back at the horn.',
      },
    },
  },
  aviary: {
...
// --- Regions -------------------------------------------------------------------
// Every node belongs to a region (used for per-region music): 'island' for the
// root, otherwise the id of its depth-1 ancestor — one of harbor, ringwood,
// lake, peaks, chancel, terraces. Assigned by walking up to the region root.

for (const node of Object.values(NODES)) {
  if (!node.parent) {
    node.region = node.id; // the root island
    continue;
  }
  let cur = node;
  while (cur.parent && cur.parent !== 'island') cur = NODES[cur.parent];
  node.region = cur.parent === 'island' ? cur.id : 'island';
}
