...
/**
 * Generate an image conditioned on a reference image. Same as generateImage,
 * but the user message carries the prompt text plus the reference image so the
 * model paints the same physical place from a new vantage. Returns a Buffer.
 */
export async function generateImageWithReference(prompt, referenceBuffer, { model } = {}) {
  const json = await post(
    {
      model: model || cfg.imageModel(),
      modalities: ['image', 'text'],
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'image_url',
              image_url: { url: `data:image/jpeg;base64,${referenceBuffer.toString('base64')}` },
            },
          ],
        },
      ],
